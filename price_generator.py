...
def create_price_tag(item_data, size_config, theme):
    ...
    if num_specs > 0:
        ...
        for i, spec in enumerate(specs):
            ...
            current_x = margin + 20
            draw.text((current_x, current_y), "• ", font=spec_font_regular, fill='black', anchor='lm')
            ...
            if ':' in spec:
                ...
                draw.text((current_x, current_y), ' ' + value.strip(), font=spec_font_regular, fill='black',
                          anchor='lm')
            else:
                draw.text((current_x, current_y), spec, font=spec_font_regular, fill='black', anchor='lm')

    # --- Draw Footer ---
    draw.line([(margin, footer_area_top), (width_px - margin, footer_area_top)], fill='black', width=3)
    footer_content_y = footer_area_top + (footer_area_height / 2)
    sku_font = get_font(PRIMARY_FONT_BOLD_PATH, 60, FALLBACK_FONT_EN_BOLD)
    draw.text((margin, footer_content_y), f"SKU: {item_data.get('SKU', 'N/A')}", font=sku_font, fill='black',
              anchor='lm')
    ...
    # UPDATED: Added .strip() to remove leading/trailing whitespace from the data file.
    sale_price = item_data.get('Sale price', '').strip()
    regular_price = item_data.get('Regular price', '').strip()

    price_x = width_px - margin
    main_price_color = theme.get('price_color', '#D32F2F')

    if sale_price and float(sale_price.replace(',', '.')) > 0:
        draw.text((price_x, footer_content_y), f"₾{sale_price}", font=price_font, fill=main_price_color, anchor='rm')
        ...
        draw.text((orig_x, footer_content_y), orig_text, font=strikethrough_font, fill='black', anchor='rm')
        draw.line([(orig_x - (orig_bbox[2] - orig_bbox[0]), footer_content_y), (orig_x, footer_content_y)],
                  fill='black', width=3)
    elif regular_price:
        draw.text((price_x, footer_content_y), f"₾{regular_price}", font=price_font, fill=main_price_color, anchor='rm')
