...
def _create_kingston_modern_tag(item_data, width_px, height_px, width_cm, height_cm, theme, language, is_special=False):
    """Creates a special price tag for Kingston with a modern, 3D design."""
    # --- Config ---
    bg_color = "#ED1C2E"
    text_color = "black"
    logo_path = theme.get('accessory_logo_path')
    ...
    if logo_path:
        try:
            ...
            print(f"Warning: Kingston logo not found at {logo_path}")
            y_cursor = content_margin
    else:
        y_cursor = content_margin
    ...
    # --- 2. Item Name ---
    name_text_raw = item_data.get('Name', 'N/A')
    # Filter out the word "Kingston"
    name_text = re.sub(r'\bKingston\b', '', name_text_raw, flags=re.IGNORECASE).strip()
    name_text = ' '.join(name_text.split())  # Clean up extra spaces
...
def create_price_tag(item_data, size_config, theme, layout_settings=None, language='en', is_special=False, background_cache=None):
    ...
    if theme.get('design') == 'kingston_modern':
        return _create_kingston_modern_tag(item_data, width_px, height_px, width_cm, height_cm, theme, language, is_special)
